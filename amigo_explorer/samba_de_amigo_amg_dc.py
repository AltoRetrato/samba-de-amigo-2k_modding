...
class SambaDeAmigoAmgDc(KaitaiStruct):
    ...
    class BlockAct(KaitaiStruct):
        ...
        def _read(self):
            self.act_num = self._io.read_u4le()
            self.num_entries = self._io.read_u4le()
            self.entries = [None] * (self.num_entries)
            for i in range(self.num_entries):
    ...
    class Command(KaitaiStruct):
        ...
        def _read(self):
            self.frame = self._io.read_u4le()
            if self.frame != 0:
                self.cmd_bitfield = self._io.read_u4le()
    ...
    class Block(KaitaiStruct):
        ...
        def _read(self):
            self.block_name = self._root.BlockNameEnum(self._io.read_u4le())
